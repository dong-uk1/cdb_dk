			...
			$.ajax({
				...
				success : function(data) {
					...
					for(var i=0; i<viewAll.length; i++) {
						var view_tmp='';
						view_tmp += "<tr>";
						view_tmp += "<td>" + viewAll[i].br_suplytype + "</td>";
						view_tmp += "<td>" + viewAll[i].br_housetype + "</td>";
						view_tmp += "<td>" + viewAll[i].br_brtc + "</td>";
						view_tmp += "<td><a href=rent_view?br_pbid=" + viewAll[i].br_pbid + "&br_brtc=" + viewAll[i].br_brtc + ">"+ viewAll[i].br_pbname + "</a></td>";
						view_tmp += "<td>" + moment(viewAll[i].br_recrude).format("YYYY-MM-DD") + "</td>";
						view_tmp += "<td>" + moment(viewAll[i].br_winannde).format("YYYY-MM-DD") + "</td>";
						view_tmp += "<td>" + viewAll[i].br_suplyint + "</td>";
						view_tmp += "</tr>";
						$("#tbody").append(view_tmp);
					}
					//$('#tbody').text(view_tmp);  //10개씩만 불러와서 테이블에 삽입하기
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////			
					if(paging.startPage != 1) {
						page_tmp += "<a href=pageList?nowPage=" + (paging.startPage - 1) + ">" + '&lt;' + "</a>";
					}
					for(var i=1; i<= paging.endPage; i++) {
						if( i === 'paging.nowPage') {
							...
							page_tmp += "<a href=pageList?nowPage=" + i + ">" + i + "</a>";
						}
					}
					if(paging.endPage != paging.lastPage){
						page_tmp +=  "<a href=pageList?nowPage=" + (paging.endPage+1)+ ">" + '&gt;' + "</a>";
					}
					$('#paging').append(page_tmp);
				} // success
			}) // ajax
		}) //click
